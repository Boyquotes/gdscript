...
var popup
var card : CardModel : set = set_card
...
# warning-ignore:return_value_discarded
	DataRepository.connect("card_updated",Callable(self,"_on_card_updated"))
# warning-ignore:return_value_discarded
	DataRepository.connect("card_created",Callable(self,"_on_card_updated"))
# warning-ignore:return_value_discarded
	DataRepository.connect("card_deleted",Callable(self,"_on_card_deleted"))
# warning-ignore:return_value_discarded
	DataRepository.connect("list_updated",Callable(self,"_on_list_updated"))
# warning-ignore:return_value_discarded
	DataRepository.connect("list_deleted",Callable(self,"_on_list_deleted"))
...
func _sync_tasks():
	...
	if has_tasks:
		for task_local in card.tasks:
			var checkitem = CheckitemScene.instantiate()
			checklist_items_container.add_child(checkitem)
			checkitem.set_model(task_local)
			checkitem.connect("edit_check_item_requested",Callable(self,"_on_edit_check_item_requested"))
			checkitem.connect("toggled",Callable(self,"_on_check_item_toggled").bind(task_local))
...
func _on_Title_gui_input(event):
	if event is InputEventMouseButton and event.get_button_index() == MOUSE_BUTTON_LEFT and not event.is_pressed():
		can_close = false
		title_edit.set_text(card.title)
...
#	checklist_items_container.add_sibling(_node, checkitem_edit_container)
