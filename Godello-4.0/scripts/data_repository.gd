...
var active_user : UserModel : set = set_active_user
var active_board : BoardModel : get = get_active_board, set = set_active_board
...
# warning-ignore:return_value_discarded
	Events.connect("user_logged_in",Callable(self,"_on_user_logged_in"))
# warning-ignore:return_value_discarded
	Events.connect("user_logged_out",Callable(self,"_on_user_logged_out"))
...
func _on_card_dropped(_drop_data, into_list):
	if _drop_data["model"].list_id != into_list.id:
		move_card_to_list(_drop_data["model"], into_list)
