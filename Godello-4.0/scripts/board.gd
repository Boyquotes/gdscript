...
var model : BoardModel : set = set_model
...
# warning-ignore:return_value_discarded
	Events.connect("card_clicked",Callable(self,"_on_card_clicked"))
# warning-ignore:return_value_discarded
	Events.connect("add_card_clicked",Callable(self,"_on_add_card_clicked"))
# warning-ignore:return_value_discarded
	DataRepository.connect("list_created",Callable(self,"_on_list_created"))
# warning-ignore:return_value_discarded
	DataRepository.connect("board_updated",Callable(self,"_on_board_updated"))
# warning-ignore:return_value_discarded
...
func set_model(_model : BoardModel):
	...
	for member in model.members:
		var button = MemberButtonScene.instantiate()
	...
	for list in model.lists:
		var list_element = ListScene.instantiate()
...
func _on_ShowMenuButton_pressed():
	...
	await menu.menu_close_requested

	tween = Tween.new()
	add_child(tween)
	tween.interpolate_property(menu,
		"offset_left", open_margin, 0, 0.2,
