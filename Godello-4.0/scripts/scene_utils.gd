...
func go_to_login():
	var err = get_tree().change_scene_to_file("res://scenes/login_register.tscn")
	if err != OK:
...
func create_single_error_popup(message : String, _focus_after_close : Control, parent : Node):
	...
	popup = load("res://scenes/single_button_popup.tscn").instantiate()
...
func create_delete_confirm_popup(parent : Node, confirm_target : Object, binds := [], title := "Are you sure?"):
	...
	dialog.connect("confirmed",Callable(confirm_target,"_on_delete_confirmed").bind(binds))
	dialog.popup()
